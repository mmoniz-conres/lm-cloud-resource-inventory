...
#-NEW-----------------------------------------------------------------------------------------
#Prep new CSV file with resources listed
output_list_csv_file="${output_csv_file%.*}-list.csv"
echo "subscription,resourcegroup,name,type,lmcategory" > "$output_list_csv_file"  #Will overwrite
#-NEW END-------------------------------------------------------------------------------------
...
do
  ...
  do
    ...
    #-NEW-----------------------------------------------------------------------------------------
    resources_with_names="$(az resource list -g $rg --query '[].{Type:type,Name:name}' -o tsv)"

    #Go thru each resource and get name and type
    if [ -n "$resources_with_names" ]; then
      while IFS=$'\t' read -r resource_type resource_name; do

        #Lookup type in above table to see how LM classifies it (IaaS, PaaS, non-compute, etc)
        ...
        if [ "$lowercase_resource_type" = "microsoft.compute/virtualmachinescalesets" ]; then
          #Get list of VMs in the VMSS
          vmss_vm_list=$(az vmss list-instances --name "$resource_name" --resource-group "$rg" --query "[].{ResourceGroup: resourceGroup, Type: type, Name:name}" -o tsv)

          #Lookup type in above table to see how LM classifies it (IaaS, PaaS, non-compute, etc), although likely always the same here
          while IFS=$'\t' read -r vmss_resource_group vmss_resource_type vmss_resource_name; do
            vmss_lowercase_resource_type=$(to_lowercase "$vmss_resource_type")
            if [ -n "${charged_datasource_dict[$vmss_lowercase_resource_type]}" ]; then
              vmsstype="${charged_datasource_dict[$vmss_lowercase_resource_type]}"
            elif [ -n "${no_charged_datasource_dict[$vmss_lowercase_resource_type]}" ]; then
              vmsstype="Non-compute"
            else
